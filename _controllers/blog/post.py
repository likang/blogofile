...
class Post(object):
    ...
    def __post_process(self):
        ...
        if not self.permalink and \
            ...
            per_fn = os.path.splitext(self.filename)[0].split('-')
            date_part = per_fn[:3]
            if len(date_part) == 3:
              try: 
               titledate = datetime.datetime(
                 int(date_part[0]),int(date_part[1]),int(date_part[2]))
               titlefn = '-'.join(per_fn[3:])
              except:
               titledate = self.date
               titlefn = '-'.join(per_fn)
            ...
            self.permalink = \
                    re.sub(":year", titledate.strftime("%Y"), self.permalink)
            self.permalink = \
                    re.sub(":month", titledate.strftime("%m"), self.permalink)
            self.permalink = \
                    re.sub(":day", titledate.strftime("%d"), self.permalink)
            self.permalink = \
